...
<div class="page-transition max-w-2xl mx-auto space-y-4">
    <x-page-title title="Tambah Polsek" subtitle="Lengkapi data polsek baru">
        <x-slot:actions>
            <x-button href="{{ route('security.polseks.index') }}" variant="outline" size="sm">← Kembali</x-button>
        </x-slot:actions>
    </x-page-title>

    <form method="POST" action="{{ route('security.polseks.store') }}" class="space-y-5 pb-4">
        @csrf
        <x-card title="Informasi Polsek" icon="🚓">
            <div class="space-y-4">
                ...
                <x-input label="Nama Polsek" name="name" required placeholder="cth: Polsek Bungku Tengah" :error="$errors->first('name')" />
                <x-input label="Alamat" name="address" placeholder="Alamat polsek" :error="$errors->first('address')" />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-input label="Jumlah Personel" name="personnel_count" type="number" min="0" :error="$errors->first('personnel_count')" />
                    <x-input label="Jumlah Poskamling" name="poskamling_count" type="number" min="0" :error="$errors->first('poskamling_count')" />
                </div>
                <div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <x-button type="submit" variant="primary">Simpan Polsek</x-button>
        </div>
    </form>
</div>
@endsection
